...
exports.run = async (client, message, args) => {
  let sunucu = client.guilds.get("753299520837910649")//TlhaMert Youtube Kanalı : https://youtube.com/c/TlhaMert
...
if(!message.member.roles.has("753301258332536963") && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send("<a:error:735845326350712932> Bu komutu kullanmaya yetkin yok.");  
 let guild = message.guild
  ...
  if(sunucu.members.get(user.id).roles.has("749915024109207634")) return message.channel.send("Bu kişiyi banlayamazsın!").then(m => m.delete(9000));
if (user == message.author) return message.channel.send(`Atacağın kişiyi etiketlemelisin kanka`)
   message.react('753494264436097075')
...
const random = new Discord.Attachment(array[Math.floor(Math.random() * array.length)])
 ...
 message.guild.member(user).ban(`${sebep} | ${yasaklayankisi}`).catch(error => message.reply("Üyeyi yasaklamak için yetkim yetmiyor."))  
    ...
    .setTimestamp()  
    let kanal1 = message.guild.channels.get("753832656185327677")
